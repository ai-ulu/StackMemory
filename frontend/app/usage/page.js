      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col gap-3 rounded-3xl border border-border/60 bg-card/60 p-5 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex gap-3">
            <Button asChild variant="outline"><Link href={withNamespaceHref('/brain', namespace)}>Open Brain</Link></Button>
            <Button asChild><Link href={withNamespaceHref('/settings', namespace)}>Memory policy</Link></Button>
          </div>
        </div>

        <Card className="border-border/60 bg-card/60">
          <CardHeader><CardTitle>Cost model roadmap</CardTitle></CardHeader>
          <CardContent className="grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
            {notes.map((note) => <div key={note} className="rounded-2xl border border-border/60 bg-background/60 p-4">{note}</div>)}
          </CardContent>
        </Card>
      </div>
    </AppShell>
