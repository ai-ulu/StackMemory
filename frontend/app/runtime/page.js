      ...
      <div className="space-y-6">
        <Card className="border-border/60 bg-card/60">
          ...
          <CardContent className="grid gap-3">
            {configRows.map(([label, value, source]) => (
        ...
        <Card className="border-border/60 bg-card/60">
          <CardHeader>
            <CardTitle>Preview another namespace</CardTitle>
          </CardHeader>
          <CardContent>
            <form className="grid gap-3 md:grid-cols-[1fr_auto]" action="/runtime">
              <Input name="namespace" placeholder="demo:stackmemory, user:123, team:alpha..." defaultValue={config.namespace} />
              <Button type="submit">Preview namespace</Button>
            </form>
            <div className="mt-4 flex flex-wrap gap-3">
              <Button asChild variant="outline">
                <Link href={withNamespaceHref('/memories', config.namespace)}>Open memories</Link>
              </Button>
              <Button asChild variant="outline">
                <Link href={withNamespaceHref('/brain', config.namespace)}>Open Brain</Link>
              </Button>
              <Button asChild>
                <Link href={withNamespaceHref('/memories/new', config.namespace)}>Create memory here</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/60 bg-card/60">
          <CardHeader>
            <CardTitle>Connection sample</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 text-sm text-muted-foreground">
            {memories.slice(0, 5).map((memory) => (
              <Link key={memory.id} href={withNamespaceHref(`/memories/${memory.id}`, config.namespace)} className="block rounded-2xl border border-border/60 bg-background/60 p-4 transition-colors hover:bg-muted/50">
                <div className="text-xs uppercase tracking-wide text-muted-foreground">{memory.type}</div>
                <div className="mt-1 text-foreground">{memory.content}</div>
              </Link>
            ))}
