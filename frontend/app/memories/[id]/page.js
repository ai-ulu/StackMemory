...
function errorMessage(error) {
  ...
  if (error === 'delete-failed') return 'Memory could not be removed through the MCP backend.';
  return null;
}
      ...
      <div className="space-y-6">
        ...
        <Card className="border-border/60 bg-card/60">
          ...
          <CardContent className="space-y-4 text-sm text-muted-foreground">
            ...
            ) : <div className="rounded-2xl border border-border/60 bg-background/60 p-4">Memory not found. This route is ready for real API-backed records.</div>}
            <div className="rounded-2xl border border-border/60 bg-background/60 p-4">Future view: related memories, graph edges and context compiler impact.</div>
          </CardContent>
        </Card>
        ...
        {memory && (
          <div className="grid gap-6 lg:grid-cols-[1fr_0.65fr]">
            <Card className="border-border/60 bg-card/60">
              <CardHeader><CardTitle>Edit memory</CardTitle></CardHeader>
              <CardContent>
                <form action={updateMemoryAction} className="space-y-5">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2"><Label htmlFor="source">Source</Label><Input id="source" name="source" defaultValue={memory.source} /></div>
                    <div className="space-y-2"><Label htmlFor="confidence">Confidence</Label><Input id="confidence" name="confidence" type="number" min="0" max="100" defaultValue={Math.round(memory.confidence * 100)} /></div>
                  </div>
            ...
            <Card className="border-destructive/30 bg-card/60">
              <CardHeader><CardTitle>Removal</CardTitle></CardHeader>
              <CardContent className="space-y-4 text-sm text-muted-foreground">
                <p>Remove this record through the MCP backend.</p>
                <form action={deleteMemoryAction}>
                  <input type="hidden" name="id" value={memory.id} />
                  {namespace && <input type="hidden" name="namespace" value={namespace} />}
                  <Button type="submit" variant="destructive" className="w-full">Remove memory</Button>
                </form>
              </CardContent>
            </Card>
          </div>
