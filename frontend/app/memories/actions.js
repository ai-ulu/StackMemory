...
export async function createMemoryAction(formData) {
  ...
  if (!content) redirect(appendParams('/memories/new', { namespace, error: 'missing-content' }));
  try {
    const memory = await createMemory({ content, type, source, importance, confidence, namespace });
...
export async function updateMemoryAction(formData) {
  ...
  const confidence = formValue(formData, 'confidence', '85');
  const namespace = formValue(formData, 'namespace');
  if (!id || !content) redirect(id ? appendParams(`/memories/${encodeURIComponent(id)}`, { namespace, error: 'missing-content' }) : appendParams('/memories', { namespace, error: 'missing-id' }));
  try {
...
export async function deleteMemoryAction(formData) {
  const id = formValue(formData, 'id');
  const namespace = formValue(formData, 'namespace');
  if (!id) redirect(appendParams('/memories', { namespace, error: 'missing-id' }));
  try {
