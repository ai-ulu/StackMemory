...
export default async function MemoriesPage({ searchParams }) {
  ...
  const memories = query ? await searchMemories({ query, type: selectedType, namespace, limit: 50 }) : await listMemories({ limit: 50, type: selectedType, namespace });
    ...
    <AppShell title="Memories" description="Search, inspect and organize reusable agent context.">
      <div className="space-y-6">
        <div className="flex flex-col gap-3 rounded-3xl border border-border/60 bg-card/60 p-5 md:flex-row md:items-center md:justify-between">
          ...
          <Button asChild><Link href={withNamespaceHref('/memories/new', namespace)}>Create memory</Link></Button>
        </div>
        ...
        <form className="grid gap-3 rounded-3xl border border-border/60 bg-card/60 p-5 md:grid-cols-[1fr_180px_auto]" action="/memories">
          ...
          <select name="type" className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" defaultValue={type}>
            {memoryTypes.map((item) => <option key={item} value={item}>{item}</option>)}
          </select>
          <div className="flex gap-2">
            <Button type="submit">Search</Button>
            <Button asChild variant="outline"><Link href={withNamespaceHref('/memories', namespace)}>Reset</Link></Button>
          </div>
        </form>
          ...
          {memories.map((memory) => (
            <Link key={memory.id} href={withNamespaceHref(`/memories/${memory.id}`, namespace)}>
              <Card className="border-border/60 bg-card/60 transition-colors hover:border-primary/40 hover:bg-card/80">
                <CardHeader className="pb-2"><CardTitle className="text-sm uppercase tracking-wide text-muted-foreground">{memory.type}</CardTitle></CardHeader>
                <CardContent className="space-y-3 text-sm">
                  <p>{memory.content}</p>
                  <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                    <span>confidence {Math.round(memory.confidence * 100)}%</span><span>importance {Math.round(memory.importance * 100)}%</span><span>decay {Math.round(memory.decay * 100)}%</span>
                  </div>
                </CardContent>
              </Card>
            </Link>
          ))}
          {!memories.length && <Card className="border-border/60 bg-card/60"><CardContent className="p-6 text-sm text-muted-foreground">No memories found for this filter.</CardContent></Card>}
        </div>
      </div>
    </AppShell>
  );
