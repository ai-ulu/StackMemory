...
export function mockMemoryFallback() {
  return mockMemories.map((memory) => ({ ...memory, source: memory.source || 'Mock fallback', raw: memory }));
}
...
export async function searchMemories({ query, limit = 10, type, namespace, fallback = true } = {}) {
  ...
  try {
    const payload = await mcp.search({ query: cleanQuery, namespace: getMemoryNamespace(namespace), limit, type });
    const memories = normalizeMemoryList(payload);
    ...
    return fallback
      ? mockMemoryFallback().filter((memory) => memory.content.toLowerCase().includes(cleanQuery.toLowerCase()) || memory.id === cleanQuery)
      : [];
  }
}
...
export async function createMemory(input) {
  ...
  const payload = await mcp.store({ content, type, confidence, importance_score, namespace, tags: source ? [`source:${source}`] : [] });
  const normalized = normalizeMemory(payload, 0);
  return normalized || { id: slugFromContent(content), type, content, confidence, importance: importance_score, decay: 0, source, raw: payload };
}
